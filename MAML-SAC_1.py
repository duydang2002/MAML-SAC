...
class Agent(object):
    ...
    def update_model(self, env, K, H, epoch):
        ...
        if (self.Type == "Defender"):
            ...
            #self.init_target_network(K)
#           tasks = self.task_selection(actions, rewards, H)
            groups = {}
            for idx, a in enumerate(actions):
                groups.setdefault(a, []).append(idx)
            tasks = sorted(groups, key=lambda a: len(groups[a]), reverse=True)[:4]
            loss_meta = 0
            for task in tasks:
                indices = np.array(groups[task])
                # Nếu số phần tử đủ H, chọn ngẫu nhiên H phần tử, còn không cho lặp lại
                if len(indices) >= K*2:
            ...
            #for act,re in zip(actions,rewards):
            #   print(act,re)
    ...
    #@tf.function
    #def get_predictions(actor_network, q0_network, q1_network, states):
     #   pis = actor_network(states, training=False)
      #  q0s = q0_network(states, training=False)
       # q1s = q1_network(states, training=False)
        #return pis, q0s, q1s
    ...
    def init_target_network(self, K,H):  
        ...
        self.target_actor_network.compile(optimizer=optimizers.Adam(0.000025),
                               loss=sac_loss)  
        ...
        self.target_q0_network.compile(optimizer=optimizers.Adam(0.000025),
        ...
        self.target_q1_network.compile(optimizer=optimizers.Adam(0.000025),
                               loss=tf.keras.losses.mse)  
        ...
        self.v_network_target.compile(optimizer=optimizers.Adam(0.000025),
                               loss=tf.keras.losses.mse) 
        ...
        self.target_v_network_target.compile(optimizer=optimizers.Adam(0.0000125),
                               loss=tf.keras.losses.mse)  
    ...
    def meta_train(self, states, actions, next_states, def_reward, dones, K):
        ...
        self.update_target_net(self.target_v_network_target,
                           self.v_network_target, 0.00005)
        return loss_actor
        

    def task_selection(self, actions, rewards, minibatch_size):
        # Tách chỉ số theo reward
        zero_reward_indices = np.where(rewards == 0)[0]  # Chỉ số của reward = 0
        one_reward_indices = np.where(rewards == 1)[0]   # Chỉ số của reward = 1

        # Số lượng mẫu có sẵn
        num_zero_available = len(zero_reward_indices)
        num_one_available = len(one_reward_indices)

        # Tính số lượng mong muốn từ mỗi nhóm
        num_select_zero = int(self.epsilon * minibatch_size)
        num_select_one = minibatch_size - num_select_zero

        # Điều chỉnh nếu không đủ mẫu
        if num_select_zero > num_zero_available:
            num_select_zero = num_zero_available
            num_select_one = minibatch_size - num_select_zero
        elif num_select_one > num_one_available:
            num_select_one = num_one_available
            num_select_zero = minibatch_size - num_select_one

        # Lấy mẫu từ reward = 0
        if num_select_zero > 0 and num_zero_available > 0:
            selected_zero_indices = np.random.choice(
                zero_reward_indices,
                size=num_select_zero,
                replace=False  
            )
        else:
            selected_zero_indices = np.array([])

        # Lấy mẫu từ reward = 1
        if num_select_one > 0 and num_one_available > 0:
            selected_one_indices = np.random.choice(
                one_reward_indices,
                size=num_select_one,
                replace=False  
            )
        else:
            selected_one_indices = np.array([])

        # Kết hợp các chỉ số đã chọn
        selected_indices = np.concatenate((selected_zero_indices, selected_one_indices)).astype(int)

        # Nếu tổng số mẫu vẫn không đủ minibatch_size, bổ sung thêm
        if len(selected_indices) < minibatch_size:
            num_missing = minibatch_size - len(selected_indices)
            # Lấy thêm mẫu từ toàn bộ tập dữ liệu có sẵn (cho phép lặp lại)
            all_indices = np.concatenate((zero_reward_indices, one_reward_indices))
            additional_indices = np.random.choice(
                all_indices,
                size=num_missing,
                replace=True  # Cho phép lặp lại để đảm bảo đủ số lượng
            )
            selected_indices = np.concatenate((selected_indices, additional_indices)).astype(int)
...
if __name__ == "__main__":
    ...
    for epoch in range(num_episodes):
        ...
        for i_iteration in range(iterations_episode):
            ...
            #print(attack, defender_actions,def_reward, att_reward, next_attack_actions)
            ...
            if ExpRep and epoch * iterations_episode + i_iteration >= minibatch_size:
                def_loss += defender_agent.update_model(env,K,H, epoch)
                att_loss += attacker_agent.update_model(env,K,H, epoch)
            elif not ExpRep:
                def_loss += defender_agent.update_model(env,K,H, epoch)
                att_loss += attacker_agent.update_model(env,K,H, epoch)
    ...
    defender_agent.actor_network.model.save_weights("models/defender_agent_model_0_1.weights.h5", overwrite=True)
    with open("models/defender_agent_model_0_1.json", "w") as outfile:
        json.dump(defender_agent.actor_network.model.to_json(), outfile)

    formated_test_path = "formated_test_adv.data"

    with open("models/defender_agent_model_0_1.json", "r") as jfile:
        model = model_from_json(json.load(jfile))
    model.load_weights("models/defender_agent_model_0_1.weights.h5")
